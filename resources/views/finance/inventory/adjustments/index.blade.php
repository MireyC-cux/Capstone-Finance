...
@section('content')
<div class="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-6" x-data="adjPage()">
  <div class="flex items-end justify-between mb-6">
    <div>
      <h1 class="text-3xl font-extrabold bg-gradient-to-r from-brand-600 to-cyan-500 bg-clip-text text-transparent">Inventory Adjustments</h1>
      <p class="text-slate-600 mt-1">Increase/Decrease stock for loss, damage, or audit corrections.</p>
    </div>
    <div class="flex items-center gap-2">
      ...
      <button @click="openCreate()" class="inline-flex items-center gap-2 rounded-xl bg-brand-600 px-4 py-2 text-white shadow hover:bg-brand-700 transition"><i class="fa fa-wrench"></i><span>New Adjustment</span></button>
    </div>
  </div>
      ...
      <table class="min-w-full text-sm">
        ...
        <tbody class="divide-y divide-slate-100">
          ...
          <tr class="hover:bg-slate-50 transition">
            <td class="px-4 py-3">{{ \Illuminate\Support\Carbon::parse($r->adjustment_date)->format('Y-m-d') }}</td>
            <td class="px-4 py-3">{{ $r->item->item_name ?? '—' }}</td>
            <td class="px-4 py-3">
              <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold border {{ $r->adjustment_type==='Increase' ? 'bg-emerald-100 text-emerald-800 border-emerald-200' : 'bg-rose-100 text-rose-800 border-rose-200' }}">{{ $r->adjustment_type }}</span>
            </td>
  ...
  <div x-show="show" x-cloak class="fixed inset-0 z-[1001] flex items-center justify-center">
    ...
    <div class="relative w-full max-w-xl rounded-2xl bg-white shadow-2xl border border-slate-200 overflow-hidden">
      <div class="flex items-center justify-between px-5 py-3 bg-gradient-to-r from-brand-600 to-cyan-500 text-white">
        <h3 class="font-semibold">New Adjustment</h3>
        <button class="opacity-90 hover:opacity-100" @click="close()"><i class="fa fa-xmark"></i></button>
      </div>
      <form method="POST" action="{{ route('finance.inventory.adjustments.store') }}" class="p-5 grid grid-cols-1 gap-4">
        @csrf
        <div>
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            ...
            <select name="adjustment_type" class="w-full rounded-xl border-slate-300 focus:border-brand-600 focus:ring-brand-600" required>
        ...
        <div class="grid grid-cols-2 gap-3">
          <div>
            <label class="block text-xs font-medium text-slate-600 mb-1">Adjustment Date</label>
            <input type="date" name="adjustment_date" value="{{ now()->toDateString() }}" class="w-full rounded-xl border-slate-300 focus:border-brand-600 focus:ring-brand-600" required />
          </div>
          <div>
            <label class="block text-xs font-medium text-slate-600 mb-1">Adjusted By (optional)</label>
            <input type="number" min="1" name="adjusted_by" class="w-full rounded-xl border-slate-300 focus:border-brand-600 focus:ring-brand-600" placeholder="employeeprofiles_id" />
          </div>
        </div>
        <div>
          <label class="block text-xs font-medium text-slate-600 mb-1">Reason</label>
          <textarea name="reason" rows="2" class="w-full rounded-xl border-slate-300 focus:border-brand-600 focus:ring-brand-600" placeholder="Describe the reason..."></textarea>
        </div>
...
<script>
function adjPage(){ return { show:false, openCreate(){ this.show=true }, close(){ this.show=false } } }
</script>
@endpush
@endsection
