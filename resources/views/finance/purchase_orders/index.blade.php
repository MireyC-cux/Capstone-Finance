...
@section('content')
<style>
  .badge-paid { background-color: #22C55E; color: white; padding: 4px 10px; border-radius: 6px; font-size: 0.85rem; font-weight: 600; display:inline-block; }
  .badge-warning { background-color: #F59E0B; color: white; padding: 4px 10px; border-radius: 6px; font-size: 0.85rem; font-weight: 600; display:inline-block; }
  .badge-default { background-color: #9CA3AF; color: white; padding: 4px 10px; border-radius: 6px; font-size: 0.85rem; font-weight: 600; display:inline-block; }
</style>
...
<div style="padding: 20px;">
  ...
  <div style="margin-bottom: 2rem;">
    <div class="d-flex justify-content-between align-items-center">
      ...
      <a href="{{ route('accounts-payable.index') }}" class="btn btn-primary" style="gap: 0.5rem;">
        <i class="fa fa-credit-card"></i><span>Accounts Payable</span>
      </a>
    </div>
  </div>
  ...
  <div class="row g-4" style="margin-bottom: 2rem;">
      ...
      <div class="card" style="min-height: 96px; padding: 1rem 1.25rem;">
        <div class="d-flex justify-content-between align-items-center">
          ...
          <div style="width: 48px; height: 48px; border-radius: 8px; background: #2563EB; color: white; display: flex; align-items: center; justify-content: center;">
            <i class="fa fa-file-signature" style="font-size: 20px;"></i>
          </div>
        </div>
      </div>
    </div>
    <div class="col-12 col-md-6 col-lg-3">
      <div class="card" style="min-height: 96px; padding: 1rem 1.25rem;">
        <div class="d-flex justify-content-between align-items-center">
          <div style="flex: 1;">
            <div style="font-size: 0.85rem; color: var(--text-secondary); font-weight: 600; text-transform: uppercase;">Unpaid</div>
           @php
$unpaidCount = $pos->filter(function($po) {
    $total = (float) $po->total_amount;

    // Determine paid amount
    $paid = 0;
    if ($po->accountsPayable) {
        $paid = (float) $po->accountsPayable->amount_paid;
    } else {
        $paid = (float) \App\Models\Expenses::where('supplier_id', $po->supplier_id)
...
<div style="font-size: 24px; font-weight: 700; color: #F59E0B; margin-top: 0.25rem;">
    {{ $unpaidCount }}
</div>
    ...
    <div class="table-responsive">
      <table class="table table-hover align-middle" style="margin-bottom: 0;">
        ...
        <tbody>
          @foreach($pos as $po)
          <tr>
            <td style="padding: 8px; font-weight: 600; font-family: ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace;">
              {{ $po->po_number }}
            </td>
            ...
            <td style="padding: 8px;">
              @php($rawStatus = $po->payment_status ?? 'Unpaid')
              @php($badgeClass = match($rawStatus) {
                  'Paid' => 'badge-paid',
            ...
            <td style="padding: 8px;">
              <div class="d-flex flex-wrap gap-2">
                <a href="#" class="btn btn-sm btn-info po-view" data-po-id="{{ $po->purchase_order_id }}" style="gap: 0.5rem;">
...
document.addEventListener('DOMContentLoaded', () => {
    const detailsModalEl = document.getElementById('poDetailsModal');
    const payModalEl = document.getElementById('poPaymentModal');
    const detailsModal = new bootstrap.Modal(detailsModalEl);
    const payModal = new bootstrap.Modal(payModalEl);
    let currentPO = null;

    // Open PO Details modal
    document.querySelectorAll('.po-view').forEach(btn => {
        btn.addEventListener('click', async (e) => {
            ...
            try {
                ...
                const poDetStatusEl = document.getElementById('poDetStatus');
                const status = po.payment_status || 'Unpaid';
                poDetStatusEl.textContent = status;
                poDetStatusEl.className = 'fw-semibold ' + (
                    status.toLowerCase() === 'paid' ? 'badge-paid' :
                    status.toLowerCase() === 'partially paid' ? 'badge-warning' : 
                    'badge-default'
                );

                // Configure Record Payment button
                const recBtn = document.getElementById('poDetRecordBtn');
                if (Number(po.outstanding) <= 0 || status.toLowerCase() === 'paid') {
                    recBtn.disabled = true;
                    recBtn.classList.add('disabled');
                    recBtn.textContent = 'Fully Paid';
                } else {
                    recBtn.disabled = false;
                    recBtn.classList.remove('disabled');
                    recBtn.textContent = 'Record Payment';
                }
    ...
    // Open Payment modal from Details modal
    ...
    pmMethod.addEventListener('change', () => {
        const isCash = (pmMethod.value === 'Cash');
        pmRef.required = !isCash;
        pmOr.required = !isCash;
    });

    // Payment type Full/Partial toggle
    const pmType = document.getElementById('poPmType');
    const pmAmount = document.getElementById('poPmAmount');
    pmType.addEventListener('change', () => {
        if (pmType.value === 'Full') {
            pmAmount.value = Number(document.getElementById('poPmMax').dataset.value).toFixed(2);
            pmAmount.readOnly = true;
        } else {
            pmAmount.value = '';
            pmAmount.readOnly = false;
    ...
    const paymentForm = document.getElementById('poPaymentForm');
    paymentForm.addEventListener('submit', (e) => {
        const max = parseFloat(document.getElementById('poPmMax').dataset.value);
        const amt = parseFloat(pmAmount.value);
        if (amt <= 0 || amt > max) {
            e.preventDefault();
            Swal.fire('Error', `Amount must be greater than 0 and not exceed ₱${max.toFixed(2)}`, 'error');
            return false;
        }
    });
});
