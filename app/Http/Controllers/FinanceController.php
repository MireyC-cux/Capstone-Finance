...
class FinanceController extends Controller
{
    ...
    public function reports(Request $request)
    {
        ...
        if ($request->filled('export') && $request->filled('type')) {
            ...
            $requested = $request->get('type');
            $types = is_array($requested) ? $requested : [(string)$requested];
            $all = ['revenue','expenses','cashflow','payroll','pnl','ar','ap'];
            if (in_array('all', array_map('strtolower', $types), true)) { $types = $all; }
            $types = array_values(array_intersect($all, array_map('strtolower', $types)));
            if (empty($types)) { $types = $all; }

            $csvFor = function(string $type) use ($revenue,$expenses,$cash,$payroll,$pnl,$ar,$ap) {
                if ($type==='revenue') return ['Revenue',['Month','Sales','Paid','Unpaid','Overdue'], array_map(fn($r)=>[$r['month'],$r['sales'],$r['paid'],$r['unpaid'],$r['overdue']], $revenue['table'])];
                if ($type==='expenses') return ['Expenses',['Month','Expense','Revenue','Expense/Revenue'], array_map(fn($r)=>[$r['month'],$r['expense'],$r['revenue'],$r['ratio']], $expenses['table'])];
                if ($type==='cashflow') return ['CashFlow',['Month','Inflow','Outflow','Net'], array_map(fn($r)=>[$r['month'],$r['inflow'],$r['outflow'],$r['net']], $cash['table'])];
                if ($type==='payroll') return ['Payroll',['Pay Period','Total','Paid','Pending'], array_map(fn($r)=>[$r['period'],$r['total'],$r['paid'],$r['pending']], $payroll['rows'])];
                if ($type==='pnl') return ['ProfitLoss',['Month','Income','Expense','Payroll','AP Payments','Net'], array_map(fn($r)=>[$r['month'],$r['income'],$r['expense'],$r['payroll'],$r['ap_payments'],$r['net']], $pnl['table'])];
                if ($type==='ar') return ['AccountsReceivable',['Customer','Total','Paid','Balance'], array_map(fn($r)=>[$r['customer'],$r['total'],$r['paid'],$r['balance']], $ar['rows'])];
                if ($type==='ap') return ['AccountsPayable',['Supplier','Total','Paid','Balance'], array_map(fn($r)=>[$r['supplier'],$r['total'],$r['paid'],$r['balance']], $ap['rows'])];
                return [ucfirst($type),[],[]];
            };

            $pdfFor = function(string $type, string $s, string $e) use ($revenue,$expenses,$cash,$payroll,$pnl,$ar,$ap) {
                ...
                $title = $titleMap[$type] ?? ucfirst($type).' Report';
                $period = $s.' to '.$e;
                $headers=[]; $rows=[]; $totals=[];
                ...
                $tbody = '';
                foreach ($rows as $r) { $tbody .= '<tr>'; foreach ($r as $cell) { $tbody .= '<td>'.htmlspecialchars((string)$cell).'</td>'; } $tbody .= '</tr>'; }
                $tfoot = '';
                if (!empty($totals)) { $colspan = max(1, count($headers)) - 1; foreach ($totals as $k=>$v) { $label = ucwords(str_replace('_',' ', (string)$k)); $val = number_format((float)$v, 2, '.', ','); $tfoot .= '<tr><td colspan="'.$colspan.'">'.htmlspecialchars($label).'</td><td style="text-align:right">'.htmlspecialchars($val).'</td></tr>'; } }
            ...
            // Single-type behaviour unchanged
            if (count($types) === 1) {
                $type = $types[0];
                if ($fmt === 'csv') {
                    [$label,$headers,$rows] = $csvFor($type);
            ...
            // Multi-type -> ZIP
            $zipBase = 'reports_'.now()->format('Ymd_His');
            $zipPath = sys_get_temp_dir().DIRECTORY_SEPARATOR.$zipBase.'.zip';
            $zip = new \ZipArchive();
            ...
            if ($fmt === 'csv') {
                foreach ($types as $t) {
                    [$label,$headers,$rows] = $csvFor($t);
                    $csvStr = fopen('php://temp','r+');
                    fputcsv($csvStr, $headers);
                    foreach ($rows as $r) { fputcsv($csvStr, $r); }
                    rewind($csvStr);
                    $data = stream_get_contents($csvStr) ?: '';
                    fclose($csvStr);
                ...
                foreach ($types as $t) {
                    [, $html] = $pdfFor($t, $s, $e);
                    $pdf = Pdf::setOptions(['defaultFont' => 'DejaVu Sans', 'isRemoteEnabled' => true])->loadHTML($html);
                    $zip->addFromString($t.'.pdf', $pdf->output());
                }
            }
            $zip->close();
